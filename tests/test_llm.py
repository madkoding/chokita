...
@patch("urllib.request.urlopen")
def test_json_decode_error_in_tool_call(mock_urlopen) -> None:
    client, memory = _make_client()
    r1 = DummyResponse('<tool>{"name": no}</tool>')
    r2 = DummyResponse("final answer")
    mock_urlopen.side_effect = [r1, r2]
...
def test_max_tool_iterations(mock_urlopen) -> None:
    client, memory = _make_client()
    tool_resp = DummyResponse('<tool>{"name":"list","args":{"path":"."}}</tool>')
    final_resp = DummyResponse("final answer")
...
@patch("urllib.request.urlopen")
def test_stream_tokens_emit_to_ui_queue(mock_urlopen) -> None:
    import queue as q
    ui = q.Queue()
    memory = Mock()
    memory.recent_messages.return_value = []
